      ...
      {isOpen && (
        ...
        >
          ...
          <div className="bg-white border-b border-black/5 px-6 py-6 flex items-center">
             <button onClick={onClose} className="p-2 hover:bg-black/5 rounded-[10px] transition-colors mr-4">
          ...
          <div className="flex-1 overflow-y-auto">
             ...
             <div className="p-6 grid grid-cols-2 gap-4">
                <div className="bg-white p-6 rounded-[10px] border border-black/5 shadow-sm">
             ...
             <div className="px-6 space-y-4 mb-6">
                <div className="relative">
                   ...
                   <input 
                      ...
                      className="w-full pl-12 pr-6 py-4 bg-white rounded-[10px] font-bold text-xs  tracking-tight outline-none border border-black/5"
                   />
                </div>
                <div className="flex bg-white p-1 rounded-[10px] border border-black/5 shadow-sm">
                   {(['ALL', 'PAID', 'UNPAID'] as const).map(f => (
                     <button 
                       key={f}
                       onClick={() => setFilter(f)}
                       className={cn("flex-1 py-2 rounded-[10px] text-[9px] font-black  tracking-tighter transition-all", filter === f ? "bg-black text-white" : "text-black/30")}
                     >
                       {f}
                     </button>
                   ))}
             ...
             <div className="px-6 space-y-4 pb-24">
                {filteredBills.length === 0 ? (
                  <div className="bg-white rounded-[10px] p-20 border-2 border-dashed border-black/5 flex flex-col items-center justify-center text-center opacity-40">
          ...
          <div className="bg-white p-6 pb-[calc(env(safe-area-inset-bottom,0px)+1.5rem)] border-t border-black/5 shadow-[0_-15px_30px_rgba(0,0,0,0.03)] z-50">
             <button 
               ...
               className="w-full bg-black text-white h-16 rounded-[10px] font-black flex items-center justify-center gap-4 shadow-2xl active:scale-95 transition-all text-sm  tracking-tight italic"
             >
                <div className="p-1.5 bg-white/20 rounded-[10px]">
